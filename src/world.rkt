...
;; function which eliminates the actors who leave the framework
...
;;function that adding a actor in the world

(define (world-add-actor wld l)
  (world (foldl cons l (world-actors wld))))

;; return health of ally-ship
...
(define (actors-collision act actors l)
  (if (null? actors) (append (list act) l)
      (let ([q (will-collide? act (car actors))])
        (cond
          [(= q 1) (append (list (actor-send act '(collide))) l (list (actor-send (car actors) '(collide))) (cdr actors))]
          [(= q 2) (append (list (actor-send act '(collide-ennemy))) l (list (actor-send (car actors) '(collide-ennemy))) (cdr actors))]
          [(= q 3) (append (list (actor-send act '(collide-obstacle))) l (list (actor-send (car actors) '(collide-obstacle))) (cdr actors))]
          [(= q 4) (append (list (actor-send act '(collide-bonus))) l (list (actor-send (car actors) '(collide-bonus))) (cdr actors))]
